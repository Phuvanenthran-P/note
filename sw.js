...
// show pinned notifications on activation
self.addEventListener('activate', (e)=>{
  e.waitUntil((async ()=>{
    // read IndexedDB to re-show pinned notifications
    try{
      const pinned = await readPinnedFromIDB();
    ...
    }catch(e){console.warn('sw activate err',e)}
  })());
});
...
self.addEventListener('notificationclick', (ev)=>{
  ...
  ev.waitUntil((async ()=>{
    ...
    // notify page
    for(const c of allClients){ c.postMessage({type:'NOTIF_CLICK',id:ev.notification.data && ev.notification.data.id}); }
  })());
});
self.addEventListener('notificationclose', (ev)=>{
  (async ()=>{
    // update IDB to mark unpinned if notification closed by user
    try{
      await markUnpinnedInIDB(ev.notification.data && ev.notification.data.id);
      const allClients = await clients.matchAll({type:'window'});
      for(const c of allClients){ c.postMessage({type:'NOTIF_CLOSED',id:ev.notification.data && ev.notification.data.id}); }
